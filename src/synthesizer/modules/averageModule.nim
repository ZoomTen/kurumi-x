...
method synthesize(module: AverageModule, x: float64, pin: int): float64 =
    var output = 0.0
    var avg = 0.0
    ...
    for link in module.inputs:
        if(link.moduleIndex > -1):
            avg += 1.0
            let moduleA = synthContext.moduleList[link.moduleIndex]
            if(moduleA == nil): continue
            output += moduleA.synthesize(x, link.pinIndex)
