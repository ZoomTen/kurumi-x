...
method synthesize*(module: PdModule, x: float64, pin: int): float64 =
    ...
    if(moduleA == nil): 
        ...
        if(x < distortX * 2 * PI): 
            ...
            return moduleA.synthesize(linearInterpolation(0, 0, distortX, distortY, x / (2 * PI)) * 2 * PI, module.inputs[0].pinIndex)
        else:
            return moduleA.synthesize(linearInterpolation(distortX, distortY, 1.0, 1.0, x / (2 * PI)) * 2 * PI, module.inputs[0].pinIndex)
