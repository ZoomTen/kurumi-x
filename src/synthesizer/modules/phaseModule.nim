...
method synthesize*(module: PhaseModule, x: float64, pin: int): float64 =
    if(module.inputs[0].moduleIndex < 0): return 0
    let moduleA = synthContext.moduleList[module.inputs[0].moduleIndex]
    if(moduleA == nil): return 0 else: return moduleA.synthesize(moduloFix(x + module.phase * 2 * PI  + module.getPhase() * PI * 2, 2 * PI), module.inputs[0].pinIndex)
