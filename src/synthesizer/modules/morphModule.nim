...
method synthesize*(module: MorphModule, x: float64, pin: int): float64 =
    var valA = 0.0
    ...
    if(moduleA != nil): valA = moduleA.synthesize(x, module.inputs[0].pinIndex)
    if(moduleB != nil): valB = moduleB.synthesize(x, module.inputs[1].pinIndex)
